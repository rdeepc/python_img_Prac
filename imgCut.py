...
for x in range(len(x_lines)):
    ...
    col_cut=plt.imshow(img[0:height_img,x_first_cut:x_secondcut], cmap=plt.get_cmap('gray'))
    y_first_cut = 0
    y_first_cut_gap = y_lines[0]
    for y in range(len(y_lines)):
        y_secondcut = int(y_first_cut + y_first_cut_gap)
        row_cut = plt.imshow(img[y_first_cut:y_secondcut, x_first_cut:x_secondcut], cmap=plt.get_cmap('gray'))
        y_first_cut = int(y_secondcut)
        plt.show()
    print(x_first_cut, x_secondcut)
